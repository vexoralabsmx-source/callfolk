...
export default function CallScreen() {
  ...
  const endCall = async () => { await end(); router.canGoBack() ? router.back() : router.replace('/(tabs)/calls'); };
  const goBack = () => router.canGoBack() ? router.back() : router.replace('/(tabs)/calls');
  const displayName = person.data?.name ?? 'Callfolk contact';
    ...
    <StandalonePanel>
      ...
      <View className="flex-1 px-5 pb-6">
        <View className="mt-2 flex-row items-center justify-between">
          <Pressable accessibilityLabel="Minimize call" onPress={goBack} className="h-12 w-12 items-center justify-center rounded-2xl bg-white/[0.07]"><ChevronDown size={22} color={colors.text} /></Pressable>
        ...
        <View className="flex-1 items-center justify-center pb-12">
          <View className="rounded-full border border-white/10 p-4"><View className="rounded-full border border-white/5 p-4"><Avatar initials={person.data?.initials ?? 'CF'} color={person.data?.color} size={132} /></View></View>
          <Text className="mt-8 text-[32px] font-semibold tracking-[-1px] text-primary">{displayName}</Text>
